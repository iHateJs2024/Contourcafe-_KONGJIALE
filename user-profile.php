<!--WELCOME TO User Profile Page CONTOUR CAFE'-->
<!-- Author: Kong Jia Le' -->
...
//!PHP TO GET nohp, email_pelanggan and alamat_pelanggan FROM pelanggan table
...
if ($result->num_rows > 0) {
  ...
  $email_current = $row['email_pelanggan'];
  $alamat_current = $row['alamat_pelanggan'];
} else {
  echo "No records found for the given idpelanggan.";
}
...
if (!isset($_SESSION['idpelanggan'])) {
  die("idpelanggan is not set in the session.");
}

$idpelanggan = $_SESSION['idpelanggan']; // Retrieve `idpelanggan` from session

// Retrieve form inputs
$namapelanggan = $_POST['username'] ?? null;
$password_pelanggan = $_POST['password'] ?? null;
...
if (!empty($password_pelanggan)) {
  ...
  if ($stmt_update_password->execute()) {
    ...
    $messages[] = "Gagal mengemas kini kata laluan!";
  }
}
...
<head>
  ...
  <script>
    // Function to alert the user when update is successful
    function showAlert(message) {
      alert(message); // JavaScript alert on update success
    }
  </script>
...
<body id="body">
  ...
  <dialog id="js-dialog-setting" class="delete-account-dialog">
    ...
    <div class="modal-scrollable-content setting">
      ...
      <div class="btn-group">
        <div class="btn-sub-group">
          <a class="btn-grid-frame" href="user-privacy-policy.html">
            <img src="icons/key-outline.svg" alt="">Dasar Privasi
          </a>
          <img src="icons/right-arrow.svg" alt="">
        </div>
        <div class="btn-sub-group">
          <a class="btn-grid-frame" href="user-terms-and-conditions.html">
  ...
  <main class="main">
    <div class="main-content-wrapper">
      <div class="nav-tab">
        <button class="nav-button">Maklumat Akaun</button>
      ...
      <p class="account-details-text-mobile">Maklumat Akaun</p>
      <div class="main-content-tab">
        <form method="post">
          <p class="account-details-text">Maklumat Akaun</p>
          <div class="text-box">
            <div class="entryarea">
              <input
                type="text"
                ...
                value="<?php echo htmlspecialchars($_SESSION['namapelanggan']); ?>" />
