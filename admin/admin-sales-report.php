                    ...
                    <div class="dynamic-order-items-container">
                      ...
                      <ol class="ul-order-items">
                        ...
                        foreach ($orders as $order) {
                          ...
                          // Get order items
                          $order_items_query = "SELECT tempahan.*, makanan.namamakanan FROM tempahan JOIN makanan ON tempahan.idmakanan = makanan.idmakanan WHERE tempahan.idpelanggan = ? AND tempahan.tarikh = ?";
                          $stmt = $conn->prepare($order_items_query);
                          $stmt->bind_param("ss", $idpelanggan, $order_date);
                          $stmt->execute();
                          $order_items_result = $stmt->get_result();

                          // Start the li tag
                          echo "<li>";
                          echo "<p>Pesanan pelanggan <span>$idpelanggan - $namapelanggan</span> pada <strong>$order_date</strong></p>";

                          // Loop through items for this order
                          while ($item = $order_items_result->fetch_assoc()) {
                            echo "<p>Item: <span>{$item['idmakanan']} - {$item['namamakanan']}</span> - Bilangan: <strong>{$item['bilangan']}</strong></p>";
                          }

                          // Close the li tag after all items are listed
                          echo "</li>";
                        }
                        ?>
                      </ol>
